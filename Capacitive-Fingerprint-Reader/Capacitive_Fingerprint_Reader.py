...
def Analysis_PC_Command():
    ...
    if  PC_Command_RxBuf[0] in ["CMD1", "1"] and Finger_SleepFlag != 1:
        print ("Number of fingerprints already available:  %d"  % GetUserCount())
    elif PC_Command_RxBuf[0] in ["CMD2", "2"] and Finger_SleepFlag != 1:
    ...
    elif PC_Command_RxBuf[0] in ["CMD3", "3"] and Finger_SleepFlag != 1:
    ...
    elif PC_Command_RxBuf[0] in ["CMD4", "4"] and Finger_SleepFlag != 1:
        ClearAllUser()
        print ("All fingerprints have been cleared !")
    elif PC_Command_RxBuf[0] in ["CMD5", "5"] and Finger_SleepFlag != 1:
    ...
    elif PC_Command_RxBuf[0] in ["CMD6", "6"]:
        if rLock.acquire(blocking=True, timeout=0.6) == True: 
            ...
            rLock.release()
    elif PC_Command_RxBuf[0] in ["CMD7", "7"]:
        r = GetFingerImage_Upload()
...
def GetFingerImage_Upload():
    ...
    r = TxAndRxCmd(command_buf, 4619, 5)
    if r == ACK_FAIL:
        print ("Fail")
        return ACK_FAIL
    elif r == ACK_TIMEOUT:
        ...
        with open('sample_finger_img.dat', 'wb') as finger_file:
            ...
            print ("saved data file to sample_finger_img.dat")
        return ACK_SUCCESS
